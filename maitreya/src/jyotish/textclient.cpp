/*
* Bugfix for earlier ignored timezone subtraction while computing Julian Date in Maitreya TextClient
* 
* ka.shrinivaasan@gmail.com 24/03/2013 - Ka.Shrinivaasan (Krishna iResearch)
*/
...
protected:
	...
	***************************************************************/
	...
	double getDoubleFromString( wxString s )
	{
		double l;
		//cout<<"getDoubleFromString() : "<<s.mb_str()<<endl;
		s.ToDouble( &l );
		//cout<<"getDoubleFromString() : l="<<l<<endl;
		return (double)l;
	}
	...
	***************************************************************/
	void setDate( wxString s )
	{
		...
		double time;
		wxString timezone1;
		const wxChar* timezone2;
		wxChar decimal='.';

		if ( ! s.IsEmpty())
		{
			//cout<<"date string ="<<s<<endl;
			...
			/*
			* Fixed bug to take timezone data and subtract the corresponding seconds value from time for Julian Date
			* TextClient commandline date switch takes additional token for timezone in decimal which is converted to
			* double with new function getDoubleFromString()
			* - Ka.Shrinivaasan 24March2013
			*/
			if ( t.HasMoreTokens()) timezone1 = t.GetNextToken();
			//cout<<"timezone ="<<timezone1<<endl;

			time = second + 60 * minute + 3600 * hour - getDoubleFromString(timezone1)*3600;
			time /= 3600;
