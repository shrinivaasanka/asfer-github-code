...
def RecursiveGlossOverlapGraph(text,document_id):
	definitiongraphedges=defaultdict(list)
	definitiongraphedgelabels=defaultdict(list)
	...
	if numeric_vertices==True:
		vertex_index=0
		vertexdict={}
		for v in set(nxg.nodes()):
			vertexdict[v]=vertex_index
			vertex_index += 1

		edgeslist=[]
		docidtokenized=document_id.split("/")
		rgof=open("RecursiveGlossOverlapGraph."+docidtokenized[len(docidtokenized)-1],"w")
		for e in set(nxg.edges()):
			edgeslist.append((vertexdict[e[0]],vertexdict[e[1]]))
		json.dump(edgeslist,rgof)

	if word_vertices==True:
		edgeslist=[]
		docidtokenized=document_id.split("/")
		rgof=open("RecursiveGlossOverlapGraph."+docidtokenized[len(docidtokenized)-1],"w")
		for e in set(nxg.edges()):
...
if __name__=="__main__":
	...
	file1 = open(corpus[0])
	text = file1.read()
	classified=RecursiveGlossOverlapGraph(text,corpus[0])
