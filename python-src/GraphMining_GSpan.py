...
class GSpan(object):
    ...
    def GraphEditDistance(self, graph1, graph2):
        ged=nx.graph_edit_distance(graph1, graph2)
        ged_opteditpaths=[]
        ged_opt=[]
        for p in nx.optimize_edit_paths(graph1, graph2):
            ged_opteditpaths.append(p)
        for e in nx.optimize_graph_edit_distance(graph1, graph2):
            ged_opt.append(e)
        print("Graph edit distance:",ged)
        print("Optimized edit paths:",ged_opteditpaths)
        print("Optimized Graph edit distance:",ged_opt)
        return (ged, ged_opteditpaths, ged_opt)
    ...
    def DFSCode(self, t):
        ...
        tdfs = nx.dfs_tree(t)
        sorted_edges = sorted(
            tdfs.edges(), key=operator.itemgetter(0), reverse=False)
        dfscode = ""
    ...
    def FindSmallestGraphs(self):
        print(self.graph_dataset_graph2dfscodes)
        sortedbydfscode = sorted(list(self.graph_dataset_graph2dfscodes.items(
    ...
    def FindChildren(self, s):
        ...
        for k, v in list(self.graph_dataset_graph2dfscodes.items()):
            index = k.find(sstr)
    ...
    def GraphSet_Projection(self):
        ...
        for n in range(len(graphset_SLeast)):
            g = graphset_SLeast[n]
            for e in g.edges():
                print("e:", e)
                s = e
...
if __name__ == "__main__":
    ...
    rgof = open(
        "./InterviewAlgorithm/graphmining/RecursiveGlossOverlapGraph.WebSpider-HTML.out.1")
    G9edges = json.load(rgof)
    G9.add_edges_from(G9edges)
    #print "G9edges:",G9.edges()
    rgof = open(
        "./InterviewAlgorithm/graphmining/RecursiveGlossOverlapGraph.WebSpider-HTML.out.2")
    G10edges = json.load(rgof)
    G10.add_edges_from(G10edges)
    #print "G10edges:",G10.edges()
    rgof = open(
        "./InterviewAlgorithm/graphmining/RecursiveGlossOverlapGraph.WebSpider-HTML.out.3")
    G11edges = json.load(rgof)
    G11.add_edges_from(G11edges)
    #print "G11edges:",G11.edges()
    rgof = open(
        "./InterviewAlgorithm/graphmining/RecursiveGlossOverlapGraph.WebSpider-HTML.out.4")
    G12edges = json.load(rgof)
    G12.add_edges_from(G12edges)
    #print "G11edges:",G12.edges()
    rgof = open(
        "./InterviewAlgorithm/graphmining/RecursiveGlossOverlapGraph.WebSpider-HTML.out.5")
    G13edges = json.load(rgof)
    G13.add_edges_from(G13edges)
    print("G13edges:",G13.edges())
    dataset=[G1,G2,G3,G4,G5,G6,G7,G8,G9,G10,G11,G12,G13,G14]
    print("G14edges:", G14.edges())
    gspan = GSpan(dataset)
    gspan.GraphSet_Projection()
    gspan.GraphEditDistance(G7,G8)
