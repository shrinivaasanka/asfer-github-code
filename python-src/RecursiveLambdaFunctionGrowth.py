	...
	def machine_translation(self, corenumber=3, definitiongraph=None, languagecode="en", pathsimilarity=0.8):
		...
		print "=========================="
		       ...
		       transare = self.dictionary.translate("are",languagecode)
                ...
                if self.machinetranslator=="goslate":
                    try:
		       transand = self.goslatetranslator.translate("and",languagecode)
		       transare = self.goslatetranslator.translate("are",languagecode)
                    except Exception as ex:
                       print ex
                if self.machinetranslator=="googletrans":
                    try:
			...
			for s1 in wn.synsets(e[0]):
				for s2 in wn.synsets(e[1]):
						if s1.path_similarity(s2) > pathsimilarity:
							...
							for l in lowestcommonhypernyms:
								for ln in l.lemma_names():
								       translatedtext.append(e[0]+ " " + transand + " " + e[1]+" " + transare +" "+ln+".")
...
if __name__=="__main__":
	...
	summary=lambdafn.create_summary(textread,graphtraversedsummary=True,shortestpath=True)
	print
	print summary
	print "=========================================================="
