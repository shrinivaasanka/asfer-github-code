...
import CompressedSensing

syllable_boundary = True 
...
class VowellessText:
    def __init__(self):
        ...
        prefixsuffixprob = WordSubstringProbabilities.wordprefixsuffixsubstringsprobdist()
        self.Probabilities_prefixes = prefixsuffixprob[0]
        self.Probabilities_suffixes = prefixsuffixprob[1]
        # self.likelydict=prefixsuffixprob[2]
        ...
        for k1, v1 in list(self.Probabilities_prefixes.items()):
            if maxv1 < v1:
                maxv1 = v1
                maxk1 = k1
                # maxvowel = maxk1[len(maxk1)-1]

        for k2, v2 in list(self.Probabilities_suffixes.items()):
            if maxv2 < v2:
                maxv2 = v2
                maxk2 = k2

        self.matchinglikelydict = {}
        ...
        # for k3,v3 in (self.Probabilities_prefixes.items()):
        #	for k4,v4 in (self.Probabilities_suffixes.items()):
        #		#print "k3=",k3,"; k4=",k4
        #		if len(k3) > 0 and len(k4) > 0 and (k3[len(k3)-1] == k4[0]):
        #			self.likelydict[k3[:-1]+k3[len(k3)-1]+k4[1:]] = v3 * v4

        # for k5, v5 in self.Probabilities_suffixes.items():
        #	self.likelydict[k5]=v5
        # for k6, v6 in self.Probabilities_prefixes.items():
        #	self.likelydict[k6]=v6

    def removevowels(self, w):
        wcomp = ""
        for x in w:
            if x in self.vowels:
                wcomp = wcomp + "_"
    ...
    def compute_maximumlikelihood(self, comp, suggestions=None):
        maxlikely = ""
        maxlikelyprob = 0.0
        if suggestions is not None:
            for x in suggestions:
                probability = self.prob(x, comp)
                if probability > maxlikelyprob:
                    maxlikelyprob = probability
                    maxlikely = x
        else:
            maxlikely = self.HiddenMarkovModel_MLE(comp)
        print(("comp=", comp, "; maxlikely=", maxlikely))
        return maxlikely

    #def prob(self, x, comp):
    #    if len(x) > len(comp):
    #        xptr = 0
    #        compptr = 0
    #        print("x=", x, "; comp=", comp)
    ...
    def HiddenMarkovModel_MLE(self, comp):
        ...
        print(("compsubstrings: ", compsubstrings))

        # start=0
        # i=0
        # compsubstrindex=-2
        # match=False
        # for k7, v7 in self.likelydict.items():
        #	print "k7=",k7,"; v7=",v7
        ...
        #		self.matchinglikelydict[k7] = v7
        #	match = False
        #	compsubstrindex = -2
        #	start=0
        #	i=0
        # likelydictwords = WordSubstringProbabilities.wordlikelydict(compsubstrings)
        # likelydictmatchwords = []
        # self.matchinglikelydict={}
        # for compsubstring in compsubstrings:
        #	likelydictmatchwordsfragment = [x for x in likelydictwords if x.find(compsubstring) != -1]
        #	likelydictmatchwords = likelydictmatchwords + likelydictmatchwordsfragment
        # for likelymatch in likelydictmatchwords:
        #	self.matchinglikelydict[likelymatch] = likelydictwords[likelymatch]
        self.matchinglikelydict = WordSubstringProbabilities.wordlikelydict(
            comp)

        # Returns the word with maximum likelihood probability
        # Word returned depends on the HMM priors in the hashtable
        print(("matchinglikelydict=", self.matchinglikelydict))
        maxlikelyretprob = -1.0
        maxlikelyret = ""
        for k7, v7 in list(self.matchinglikelydict.items()):
            if maxlikelyretprob < v7:
                maxlikelyret = k7
                maxlikelyretprob = v7

        print(("maxlikelyretprob=", maxlikelyretprob,
               "; maxlikelyret = ", maxlikelyret))
        print((self.matchinglikelydict))
        return maxlikelyret
    ...
    def prob(self, x, comp):
        ...
        setdiffconso = [y for y in setdiff if y not in self.vowels]
        print(setdiffconso)
...
if __name__ == "__main__":
    ...
    vowellesstxt = VowellessText()
    csens = CompressedSensing.CompressedSensing()
    origfilesize = 0
    compfilesize = 0
    compbits = 0
    ...
    for i in f:
        if syllable_boundary == False:
            wcomp = vowellesstxt.removevowels(i)
        else:
            wcompcsens = csens.syllable_boundary_text_compression(i)
            wcomp = " ".join(wcompcsens[2].split("-"))
        print(("wcomp:", wcomp))
        ...
        for n in wcomp:
            fcomp.write(n)
        print(("Compression ratio = ", 100 *
               (origfilesize-compfilesize-0.125*compbits)/origfilesize, "%"))
    fcomp.close()
    ...
    # Decompression
    print("Decompression using Hidden Markov Model or PyEnchant Spellcheck suggest() function:")
    for i in fcompstr.split():
        suggestions = d.suggest(i)
        print((i, suggestions))
        midpoint = len(suggestions)
        maximumlikelihoodword_enchant = vowellesstxt.compute_maximumlikelihood(
            i, suggestions)
        maximumlikelihoodword_HMM = vowellesstxt.compute_maximumlikelihood(i)
        print("maximumlikelihoodword - PyEnchant :",maximumlikelihoodword_enchant)
        print("maximumlikelihoodword - HMM :",maximumlikelihoodword_HMM)
        fdecomp.write(maximumlikelihoodword_HMM)
        fdecomp.write(" ")
