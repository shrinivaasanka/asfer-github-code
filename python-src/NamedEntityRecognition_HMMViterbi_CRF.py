...
start_probabilities = {'noun': 0.3, 'verb': 0.2, 'object': 0.2,
                       'adjective': 0.1, 'adverb': 0.1, 'conjunction': 0.2}
...
for s in states:
    ...
    x = np.linspace(skewnorm.ppf(0.01, 5, cnt*0.5, 1),
                    skewnorm.ppf(0.99, 5, loc=cnt, scale=1), len(observations))
...
for s in states:
    ...
    Viterbi[0][s] = start_probabilities[s] * \
        emission_probabilities[s][observations[0]]
    Viterbi_path[s] = [s]
...
for x in range(1, len(observations)):
    ...
    for y in states:
        (probability, state) = max(
            (Viterbi[x-1][t]*transition_probabilities[t][y], t) for t in states)
...
for t in zip(missing_PoS, prev_PoS_tag):
    ...
    for s in states:
        ...
        feature[s] = emission_probabilities[s][t[0]] * \
            transition_probabilities[t[1]][s]
        PoS_probabilities[s] = k*math.exp(weight*feature[s])
        print("argmax() computation for observations missing PoS tags: emission(s=", s, ",t[0]=", t[
            0], ")*transition(t[1]=", t[1], ",s=", s, ") - feature[s]=", feature[s], ", PoS_probabilities[s] =", PoS_probabilities[s])
...
#####################################################################################################################################
print("observations text:"," ".join(observations))
nertg = RecursiveGlossOverlapGraph(" ".join(observations))
print("nertg=", nertg[0].edges())
numvertices=len(nertg[0].nodes())-1
queryweights = np.ones((numvertices,numvertices)).tolist()
keyweights = np.ones((numvertices,numvertices)).tolist()
valueweights = np.ones((numvertices,numvertices)).tolist()
variables = np.ones((numvertices,numvertices)).tolist()
rlfg=RecursiveLambdaFunctionGrowth()
rlfg.rlfg_transformers_attention_model(
    definitiongraph=nertg[0], query_weights=queryweights, key_weights=keyweights, value_weights=valueweights, variables=variables)
obs_states = list(zip(observations, PoS_CRF_states))
os = 0
for obs_state in obs_states:
    ...
    if obs_state[1] == "?":
        ...
        print("PoS Trellis Cartesian Paths - All possible PoS Paths between PoS annotated and unannotated words - ", obs_states[
            os-1][0], " and ", obs_states[os][0], ":")
        penultimate_pos = []
        ...
        print("Majority PoS of Penultimate PoS in Pos Trellis:", Counter(
            penultimate_pos).most_common())
    os += 1
