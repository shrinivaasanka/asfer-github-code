...
static PyObject* asfer_virgo_set_kernel_analytics(char* key, char* value)
{
	switch(memory_versus_filesystem)
	{
		...
		case 1:
		{
			...
			*/
			...
			syscall(556,vfsdesc,"test_virgo_filesystem_20April2017",250,0); /* write */

			/* syscall(365,vfsdesc);  */ /* close */
			syscall(554,vfsdesc);  /* close */

			return Py_BuildValue("fd",vfsdesc);
		}
	}
}
