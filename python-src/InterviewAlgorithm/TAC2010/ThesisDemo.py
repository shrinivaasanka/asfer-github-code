...
def update_reference_with_candidate(reference,candidate,refdefgraph,canddefgraph,value_addition):
	...
	for kw, p, lev in refdefgraph:
		sentences_with_kw = get_sentences_from_candidate_and_reference(kw, reference, candidate)
...
def get_sentences_from_candidate_and_reference(v, reference, candidate):
	ref_file = open(reference[0])
	ref_file_contents = ref_file.read()
	...
	refsentences_with_v = [(sentence_score(x), x) for x in refsentences if x.find(v) != -1]
	candsentences_with_v = [(sentence_score(x), x) for x in candsentences if x.find(v) != -1]
	refsentences_with_v.extend(candsentences_with_v)
	#print "refsentences=",
	#print refsentences_with_v
	return refsentences_with_v	
...
references = ['ThesisDemo-sanskrit-test1.txt']
candidate = ['ThesisDemo-sanskrit-test5.txt']
