...
picklecache = base.Client(("127.0.0.1",11211))
...
def asfer_pickle_string_load(picklef):
    ...
    #keyword = picklecache.get(picklef)
...
def asfer_pickle_dump(prevlevelsynsets, picklef):
    ...
    #if len(concat) > 0:
    #    try:
    #        todump=str(picklecache.get(picklef)) + concat
    #        picklecache.set(picklef,todump)
    #    except:
    #        print("Exception - todump:",todump)
    #picklelock.release()
...
def asfer_pickle_load(picklef):
    ...
    #line = str(picklecache.get(picklef))
    synsets = []
    print("asfer_pickle_load() - line:",line)
...
def mapFunction2(keyword):
    ...
    if cached_mapped_object != "Novalue":
        ...
        picklef = open("RecursiveGlossOverlap_MapReduce_Persisted.txt", "a")
        asfer_pickle_dump(prevlevelsynsets, picklef)
        #asfer_pickle_dump(prevlevelsynsets, "RecursiveGlossOverlap_MapReduce_Persisted.txt")
...
def Spark_MapReduce(level, wordsatthislevel, graphcache):
    ...
    md5hash = hashlib.md5(",".join(wordsatthislevel).encode("utf-8")).hexdigest()
    ...
    if cachevalue:
        ...
        spcon = SparkContext("local[4]", "Spark_MapReduce")
...
def mapFunction_Parents(prevleveltokens):
    ...
    #keyword = asfer_pickle_string_load("RecursiveGlossOverlap_MapReduce_Parents_Persisted.txt")
...
def Spark_MapReduce_Parents(keyword, tokensofprevlevel, graphcache):
    ...
    md5hashparents = hashlib.md5(keyword.encode("utf-8")).hexdigest()
    ...
    #asfer_pickle_string_dump(keyword, "RecursiveGlossOverlap_MapReduce_Parents_Persisted.txt")
    picklef_keyword.close()
    ...
    if cachevalue:
        ...
        # picklelock.acquire()
        spcon = SparkContext("local[4]", "Spark_MapReduce_Parents")
