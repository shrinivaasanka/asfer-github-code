...
def pressure_gradient(variablegravities=[],seasurfacelonglat=None,datefrom=None,dateto=None,userdefinedsst=None,friction=None,doeVelocityBydt=None,Velocity=None,VelocityGradient=None,Omega=None):
    ...
    pressuregradients=[]
    cnt=0
    print("userdefinedsst:",userdefinedsst)
    while date != todate:
        if userdefinedsst is None:
            import s3fs
...
def climate_analytics(datasource,date="",time="",predict_EWE_params=None,precipitation_timeseries=None,computepressuregradient=False,seasonaltemperature=25):
        ...
        if predict_EWE_params is not None:
            ...
            for bp in bodypairs:
                ...
                if computepressuregradient:
                    sstarray=np.zeros(len(gravityl2norms[0]))
...
if __name__ == "__main__":
    ...
    climate_analytics(datasource="precipitation_GaussianMixture",predict_EWE_params={'datefrom':(2026,10,1,17,30,00),'dateto':(2026,11,30,17,30,00),'loc':'@0','bodyconjunctions':"Mercury-Venus",'angularsepbounds':('0d','30d'),'longlat':['223@399'],'seasurfacelonglat':(95,12)},precipitation_timeseries={"timeseries":nem_rainfall_timeseries,"averageseasonalrainfall":100,'forecast_timeseries':[10,2,12,25,30,1,2,3,4,10]},computepressuregradient=True,seasonaltemperature=25)
