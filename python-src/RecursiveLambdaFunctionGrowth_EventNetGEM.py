...
class RecursiveLambdaFunctionGrowth_EventNetGEM(object):
    def __init__(self):
    ...
    def grow_lambda_function2(self, wordlist):
        ...
        while len(self.lambda_expression) > 2:
            ...
            self.lambda_composition = "("+function + \
                "("+operand1+","+operand2+"))"
    ...
    def get_tensor_neuron_potential_for_relation(self, synset_vertex, synset_r):
        ...
        for s1, s2 in product(synset_vertex, synset_r):
            ...
            if smt is not None and smt > similarity and smt != 1.0:
                ...
                for e in kcore.edges():
                    for s1 in wn.synsets(e[0]):
                        for s2 in wn.synsets(e[1]):
                            ...
                            s2dictkeys = list(s2dict.keys())
                            for s, d in s1dict.items():
                                if s in s2dictkeys:
                    ...
                    if len(intermediates) > 3:
                        ...
                        for i in range(len(intermediates)-2):
                            sentence2 = intermediates[i] + \
                                " is a " + intermediates[i+1] + "."
                            if sentence2 not in summary:
                                summary[sentence2] = self.relevance_to_text(
                                    sentence2, text)
                        sentence3 = intermediates[len(
                            intermediates)-1] + " is a " + e[1]
                        summary[sentence3] = self.relevance_to_text(
                            sentence3, text)
                        intermediates = []
                sorted_summary = sorted(
                    summary, key=operator.itemgetter(1), reverse=True)
                print("===================================================================")
    ...
    def rlfg_eventnet_graphical_event_model(self,definitiongraph):
        verbpaths=[]
        top_core_words=[]
        definitiongraph.remove_edges_from(nx.selfloop_edges(definitiongraph))
        sorted_core_nxg = sorted(list(nx.core_number(definitiongraph).items()), key=operator.itemgetter(1), reverse=True)
        for w in sorted_core_nxg:
    ...
    def grow_lambda_function3(self, text="None", level=3, textgraph=None):
        ...
        sentiment = SentimentAnalyzer.SentimentAnalysis_RGO_Belief_Propagation_MarkovRandomFields(
            definitiongraph)
        self.rlfg_eventnet_graphical_event_model(definitiongraph)
        apsp = nx.all_pairs_shortest_path(definitiongraph)
    ...
    def machine_translation(self, corenumber=3, definitiongraph=None, languagecode="en", pathsimilarity=0.8):
        ...
        # Hypergraph transversal. Other measures create a summary too : Vertex Cover is NP-hard while Edge Cover is Polynomial Time.
        kcore = nx.k_core(definitiongraph, corenumber)
        print("Text Translated by k-core(subgraph having vertices of degree atleast k) on the Recursive Gloss Overlap graph:")
        print("==========================")
        print("Dense subgraph edges:")
        print("==========================")
        print(kcore.edges())
        ...
        if self.machinetranslator == "pydictionary":
            try:
                transand = self.dictionary.translate("and", languagecode)
                transare = self.dictionary.translate("are", languagecode)
            except Exception as ex:
                print(ex)
        if self.machinetranslator == "goslate":
            try:
                transand = self.goslatetranslator.translate(
                    "and", languagecode)
                transare = self.goslatetranslator.translate(
                    "are", languagecode)
            except Exception as ex:
                print(ex)
        if self.machinetranslator == "googletrans":
            try:
                transand = self.googletranslator.translate("and", languagecode)
                transare = self.googletranslator.translate("are", languagecode)
            except Exception as ex:
                print(ex)
        for e in kcore.edges():
            for s1 in wn.synsets(e[0]):
    ...
    def korner_entropy(self, definitiongraph):
        ...
        for v in nodes:
            try:
                stable_sets.append(nx.maximal_independent_set(
                definitiongraph.to_undirected(), [v]))
            except:
                continue
...
if __name__ == "__main__":
    lambdafn = RecursiveLambdaFunctionGrowth_EventNetGEM()
    text = open("RecursiveLambdaFunctionGrowth_EventNetGEM.txt", "r")
    textread = text.read()
